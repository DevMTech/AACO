...
SoftwareSerial mySerial(2, 3);  // pro min // D2=RX, D3=TX
//SoftwareSerial mySerial(10, 11); // Uno // RX, TX

//SoftwareSerial mySerial(14, 12, false, 256); // D5(RX), D6(TX) [ESP8266]
...
/*
    if (Serial.available())   // WRITE
    { Serial.print("\nWRITING ... ");
      mySerial.print(Serial.readString());  // mySerial.write(Serial.read());
    }  

    if (mySerial.available()) // READ
    {    
      Serial.print("\nRESPONSE: ");
      Serial.println(mySerial.readString());   // Serial.write(mySerial.read());
    }
*/
