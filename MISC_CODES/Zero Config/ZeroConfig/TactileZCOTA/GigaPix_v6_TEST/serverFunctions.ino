    ...
    for (int i = 0; i < n; ++i)
     {
      ...
      st +="<li>";
      st += "<label>"; //       
      st +=i + 1;      st += ": ";      st += WiFi.SSID(i);      st += " (";      st += WiFi.RSSI(i);      st += ")";      st += (WiFi.encryptionType(i) == ENC_TYPE_NONE)?" (OPEN)":"*";
      st += "</label>"; // "</li>";
      st +="</li>";
      delay(10);
     }
...
//            server.on("/configParams", handleConfigParamsRoot);
            ...
            String actionPath = "/"+euuid+"/actionPanel";
            const char * cActionPath = (char*) actionPath.c_str();
            server.on(cActionPath, handleActionConfigSave);
...
//  s = "Configuration of " + hostName + " at ";
  s += "\r\n\r\n<p> Configuration of   " + hostName + "   at   IP :   ";
  
            /*****************************************************************************************************/
  //s += "\r\n\r\n<p><a href=\"/gpio\">Control GPIO</a><br />";
  
//  s += "\r\n\r\n<p><a href=\"/configParams\">Edit Config Parameters</a><br />";     // Edit Config Parameters
  ...
  if (WiFi.status() != WL_CONNECTED)
  {
      ...
      s += ipStr;
      s += st;                     // st is generated inside setupAP()                   // st -> List of available WiFi Hotspots
...
//  s += "<form method='get' action='a'><fieldset><legend>Configuration Parameters</legend><ul class=form-fields>";
  ...
  s += "<form method='get' action='b'><fieldset><legend>Configuration Parameters</legend><ul class=form-fields>";
  s += "</br>Please update the fields as per requirement:</br></br>";   
  s += "<label>SSID: </label><input name='ssid' length=32></br></br><label> Pass: </label><input name='pass' type='password' length=64></br></br>";
  
//  s += "<label>IOT mode: </label><input type='radio' name='iot' value='0'> HTTP<input type='radio' name='iot' value='1' checked> MQTT</br>";
  
  s += "<label>PLATFORM URL [http://10.208.35.169:3300]: </label><input name='platformUrl' length=64></br></br>";
//  s += "<label>PORT [3300]: </label><input name='restPort' length=8></br>";
  s += "<label>APPLICATION SERVER URL [http://10.208.26.254:81/gigapan/index.php]: </label><input name='serverUrl' length=64></br></br>";
...
//  s += "<input type='submit'></form></p>";
          ...
          /*****************************************************************************************************/
//// otaServerIndex = "<form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update'><input type='submit' value='Update'></form>";
//  s = "\r\n\r\n<form><fieldset><legend>OTA configuration</legend><ul class=form-fields>"; //"<ul>";  
//  s += "</br>Please update the fields as per requirement:</br></br>";
//  s +=otaServerIndex;
//  s += "</ul></fieldset></form></p>";
////  s += "</ul></fieldset></form>\r\n\r\n";  //"</ul>";
          /*****************************************************************************************************/
  
  s += "</br><a href=\"/cleareeprom\">Clear settings and boot into Config mode</a><br />";
...
void webHandleClearRom(){
  String s;
  s = "<p>Clearing the config and resetting<p>";
  s += "</html>\r\n\r\n";
  Serial.println("Sending 200"); 
  server.send(200, "text/html", s); 
