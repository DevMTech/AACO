/*
  ...
  I2C device found at address 0x5A  ! // CCS811
...
// CCS811(int nwake=-1, int slaveaddr=CCS811_SLAVEADDR_0); 
...
// CCS811 ccs811(-1); // nWAKE to GND, I2C default address 0x5A
...
//  Serial.begin(115200); // Enable serial
  ...
  if (eCO2 > 0 && eCO2 < 1000) Serial.println("NORMAL CO2 concentration...\n");
  if (eCO2 > 1000 && eCO2 < 2000) Serial.println("HIGH CO2 concentration... can cause drowsiness, poor concentration!\n");
  if (eCO2 > 2000) Serial.println("DANGEROUSLY HIGH CO2 concentration... can cause loss of attention, Headaches, sleepiness, increased heart rate!!!\n");
...
{
  // Read
  uint16_t e_total_VOC, errstat, raw; // eCO2, 
  ccs811.read(&eCO2,&e_total_VOC,&errstat,&raw); 
...
//    Serial.print("Total eVOC="); Serial.print(e_total_VOC);    Serial.print(" ppb  ");
